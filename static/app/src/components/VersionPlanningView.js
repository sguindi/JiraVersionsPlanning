...
import { cascadePlan, detectConflicts, calcEndDate, buildWorkingDays, findCriticalPath } from '../utils/planning';
...
export default function VersionPlanningView({ projectKeys }) {
  ...
  const computedPlan = useMemo(() => cascadePlan(plan, roughMap), [plan, roughMap]);
  const conflicts = useMemo(() => detectConflicts(computedPlan, roughMap), [computedPlan, roughMap]);
  ...
  const rows = useMemo(() => {
    ...
    return result;
  }, [epics, storiesByEpic, expandedEpics]);

  const workingDays = useMemo(() => buildWorkingDays(planStart, 90), [planStart]);
  ...
  function togglePlaceholder(issueKey, phId) {
    ...
    const updated = current.includes(phId) ? current.filter(id => id !== phId) : [...current, phId];
    updateIssueEntry(issueKey, { assignedPlaceholders: updated });
  }
              ...
              {rows.map((row, rowIdx) => {
                ...
                return (
                  <div key={row.key} style={{ height: ROW_HEIGHT, borderBottom: '1px solid #F4F5F7', position: 'relative', background: rowIdx % 2 === 0 ? '#fff' : '#FAFBFC' }}>
                    ...
                    {/* Gantt bar: stories use own bar; epics use summary bar */}
                    {row._isEpic ? (() => {
